...
import { UserCommentItem } from '@/components/models/user-comment/user-comment-item';
...
//
export type UserCommentListProps = {
  username: string;
  user: User;
};
// ___________________________________________________________________________
//
export const UserCommentList: React.VFC<UserCommentListProps> = React.memo(({ username, user }) => {
      ...
      <>
        <div className='mt-3 bg-white divide-y divide-gray-100 rounded-lg'>
          {comments.map((comment) => (
            <UserCommentItem key={comment.id} comment={comment} user={user} />
          ))}
        </div>
        {nextPage && (
...
// ___________________________________________________________________________
//
UserCommentList.displayName = 'UserCommentList';
